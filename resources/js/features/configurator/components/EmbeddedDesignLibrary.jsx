...
export default function EmbeddedDesignLibrary({
    ...
    const openShopifyAccount = () => {
        window.top.location.assign(storefront?.shopifyAccountUrl ?? "/account");
    };

    return (
        <div className="min-h-dvh bg-[#f8f9fc] text-slate-950">
            <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
                <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            type="button"
                            className="hidden h-11 items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3.5 text-sm font-semibold text-indigo-700 sm:inline-flex"
                        >
                        ...
                        <button
                            type="button"
                            onClick={onCreateDesign}
                            className="hidden h-11 items-center gap-2 rounded-xl px-3.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 md:inline-flex"
                        >
                        ...
                        <button
                            ...
                            className="inline-flex h-11 items-center gap-2 rounded-xl px-3.5 text-sm font-semibold text-slate-600 transition hover:bg-rose-50 hover:text-rose-600"
                        >
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
                <div className="relative flex min-h-[250px] flex-col justify-center overflow-hidden rounded-[28px] border border-indigo-100 bg-[linear-gradient(112deg,#fff_0%,#fbfbff_51%,#eef1ff_100%)] p-7 shadow-[0_14px_38px_rgba(49,46,129,0.06)] sm:p-10">
                    ...
                    <div className="relative z-10 max-w-xl">
                        ...
                        <h1 className="mt-3 text-4xl font-black tracking-[-0.035em] sm:text-[42px]">
                            Hello, {customer?.name}
                        </h1>
                        <p className="mt-3 text-sm leading-6 text-slate-500">
                            Continue a draft or bring a new garment idea to life
                            in your private design workspace.
                        </p>
                        <button
                            type="button"
                            onClick={onCreateDesign}
                            className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:shadow-xl"
                        >
                            <UiIcon name="plus" className="h-4 w-4" />
                            Create new design
                        </button>
                ...
                <div className="mt-9 flex items-end justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Everything you have saved, ready to continue.
                        </p>
                    </div>
                    {designs.length > 0 && (
                ...
                {loading ? (
                    ...
                    <section className="mt-5 rounded-[28px] border border-dashed border-slate-300 bg-white/90 px-6 py-16 text-center shadow-sm">
                        ...
                        <h2 className="mt-5 text-xl font-black">
                            Your first design starts here
                        </h2>
                        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                            Choose a published garment, personalize it, then save
                            it as a draft or finish it.
                        </p>
                        <button
                            type="button"
                            onClick={onCreateDesign}
                            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-600/20"
                        >
                            <UiIcon name="sparkles" className="h-4 w-4" />
                            Open configurator
                        </button>
                    ...
                    <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                        {designs.map((design) => (
                            <SavedDesignCard
                                ...
                                embedded
                                onTogglePreview={() =>
                                    setPreviewDesignId((id) =>
                                        id === design.id ? null : design.id,
                                    )
                                }
