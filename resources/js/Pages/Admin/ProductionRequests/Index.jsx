...
export default function Index({ requests, filters = {}, counts }) {
    const cards = [
        ...
        ['Customer decision', counts.awaitingCustomer],
                ...
                {requests.data.length === 0 ? (
                    ...
                    <div className="divide-y divide-slate-100">
                        {requests.data.map((item) => (
                            <Link key={item.id} href={route('admin.production-requests.show', item.id)} className="grid gap-3 px-5 py-5 transition hover:bg-slate-50 sm:grid-cols-[1.4fr_1fr_.7fr_.8fr_auto] sm:items-center">
                                <div>
                                    <p className="font-black text-slate-950">{item.designName}</p>
                                    <p className="mt-1 text-xs text-slate-500">
                                        {item.productName} · {item.quantity} unit{item.quantity === 1 ? '' : 's'}
                                    </p>
                                </div>
                                <div>
