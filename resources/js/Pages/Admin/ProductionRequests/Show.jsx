...
const labels = {
    ...
    completed: 'Completed',
    cancelled: 'Cancelled',
    ordered: 'Shopify order received',
};
const actionLabels = {
    review: 'Start review',
    request_changes: 'Request changes',
    reject: 'Reject request',
    mark_paid: 'Mark payment cleared',
    proof_review: 'Request proof approval',
    ready_for_print: 'Approve for printing',
...
export default function Show({ productionRequest: item }) {
    const quote = useForm({
        ...
        shipping: item.quote?.shipping ?? 0,
        tax: item.quote?.tax ?? 0,
        discount: item.quote?.discount ?? 0,
        currency: item.quote?.currency ?? 'USD',
        notes: item.quote?.notes ?? '',
        expires_at: item.quote?.expiresAt?.slice(0, 10) ?? '',
    });
    ...
    const sendReply = (event) => {
        ...
        reply.post(route('admin.production-requests.messages.store', item.id), {
            preserveScroll: true,
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(320px,.7fr)]">
                <div className="space-y-6">
                    ...
                    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
                            ...
                            <p className="mt-2">
                                <strong>Pattern:</strong> {item.summary?.pattern}
                            </p>
                            <p className="mt-2">
                                <strong>Artwork:</strong> {item.summary?.artwork}
                            </p>
                        </div>
                    ...
                    {['under_review', 'changes_requested', 'quoted'].includes(item.status) && (
                        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                            ...
                            <form
                                onSubmit={(event) => {
                                    event.preventDefault();
                                    quote.post(route('admin.production-requests.quote', item.id), { preserveScroll: true });
                                }}
                                className="mt-5 grid gap-4 sm:grid-cols-2"
                            >
                                <Field label="Unit price" type="number" value={quote.data.unit_price} onChange={(v) => quote.setData('unit_price', v)} />
                                <Field label="Currency" value={quote.data.currency} onChange={(v) => quote.setData('currency', v.toUpperCase())} />
                                <Field label="Shipping" type="number" value={quote.data.shipping} onChange={(v) => quote.setData('shipping', v)} />
                                <Field label="Tax" type="number" value={quote.data.tax} onChange={(v) => quote.setData('tax', v)} />
                                <Field label="Discount" type="number" value={quote.data.discount} onChange={(v) => quote.setData('discount', v)} />
                                <Field label="Expires" type="date" value={quote.data.expires_at} onChange={(v) => quote.setData('expires_at', v)} />
                ...
                <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between gap-3">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-wider text-violet-600">Shared conversation</p>
                            <h2 className="mt-1 text-xl font-black">Messages & activity</h2>
                        </div>
                    ...
                    <div className="mt-6 max-h-[640px] space-y-5 overflow-y-auto pr-1">
...
function Field({ label, value, onChange, type = 'text' }) {
    return (
        <label>
            ...
            <input type={type} min={type === 'number' ? 0 : undefined} step={type === 'number' ? '0.01' : undefined} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1.5 h-11 w-full rounded-xl border-slate-300" />
        </label>
    );
}
