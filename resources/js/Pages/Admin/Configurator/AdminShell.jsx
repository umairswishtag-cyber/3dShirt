...
export default function AdminShell({ title, subtitle = null, children, actions = null, hero = null, compact = false }) {
    const { flash = {}, auth = {}, chatAlerts = {} } = usePage().props;
    const { url } = usePage();
                ...
                <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
                    <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
                        <Link href={route('admin.dashboard')} className="flex min-w-0 items-center gap-3">
                            <span className="relative grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-400 via-indigo-500 to-violet-600 text-base font-black text-white shadow-lg shadow-indigo-500/20">
                                <span className="absolute -right-2 -top-2 h-7 w-7 rounded-full bg-white/20 blur-sm" />
                                <span className="relative">3D</span>
                            </span>
                            <span className="hidden min-w-0 sm:block">
                                <strong className="block truncate text-base font-bold tracking-tight">Configurator admin</strong>
                        ...
                        <nav aria-label="Admin navigation" className="flex min-w-0 items-center gap-0.5 sm:gap-1">
                            <Link aria-current={url === '/admin' ? 'page' : undefined} href={route('admin.dashboard')} className={navClass(url === '/admin')}>
                                <UiIcon name="dashboard" className="h-[18px] w-[18px]" />
                                <span className="hidden md:inline">Dashboard</span>
                            </Link>
                            <Link aria-current={url.startsWith('/admin/configurator/products') ? 'page' : undefined} href={route('admin.configurator.products.index')} className={navClass(url.startsWith('/admin/configurator/products'))}>
                                <UiIcon name="products" className="h-[18px] w-[18px]" />
                                <span className="hidden md:inline">Products</span>
                            </Link>
                            <Link aria-current={url.startsWith('/admin/production-requests') ? 'page' : undefined} href={route('admin.production-requests.index')} className={`${navClass(url.startsWith('/admin/production-requests'))} relative`}>
                                <UiIcon name="chat" className="h-[18px] w-[18px]" />
                                <span className="hidden md:inline">Requests</span>
                                {chatAlerts.unreadCount > 0 && (
                                    <span className="absolute -right-1.5 -top-1.5 grid min-h-5 min-w-5 place-items-center rounded-full bg-rose-600 px-1 text-[10px] font-black text-white ring-2 ring-white" aria-label={`${chatAlerts.unreadCount} unread messages`}>
                                        {chatAlerts.unreadCount > 99 ? '99+' : chatAlerts.unreadCount}
                                    </span>
                                )}
                            </Link>
                            <Link aria-current={url.startsWith('/admin/orders') ? 'page' : undefined} href={route('admin.orders.index')} className={navClass(url.startsWith('/admin/orders'))}>
                                <UiIcon name="orders" className="h-[18px] w-[18px]" />
                                <span className="hidden md:inline">Orders</span>
                            </Link>
                            <Link aria-current={url.startsWith('/admin/customers') ? 'page' : undefined} href={route('admin.customers.index')} className={navClass(url.startsWith('/admin/customers'))}>
                                <UiIcon name="users" className="h-[18px] w-[18px]" />
                                <span className="hidden lg:inline">Customers</span>
                            </Link>
                            <Link aria-current={url.startsWith('/admin/configurator/catalog-options') ? 'page' : undefined} href={route('admin.configurator.taxonomies.index')} className={navClass(url.startsWith('/admin/configurator/catalog-options'))}>
