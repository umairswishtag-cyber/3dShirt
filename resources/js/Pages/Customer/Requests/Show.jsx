...
const labels = {
    ...
    ready_for_print: 'Approved for print',
...
export default function Show({ productionRequest: item, storefront }) {
    const { flash = {}, chatAlerts = {} } = usePage().props;
    ...
    useEffect(() => {
        const timer = window.setInterval(
            () =>
                router.reload({
                    only: ['productionRequest', 'chatAlerts'],
                    preserveScroll: true,
                }),
            15000,
        );
    ...
    const respond = (action, note = null) =>
        router.post(
            route('store.customer.requests.respond', {
                store: storefront.key,
                id: item.id,
            }),
            { action, note },
            { preserveScroll: true },
    ...
    const sendReply = (event) => {
        ...
        reply.post(
            route('store.customer.requests.messages.store', {
                store: storefront.key,
                id: item.id,
            }),
            {
    ...
    const quoteDecisionPending = Boolean(item.quote) && item.paymentStatus === 'not_requested' && ['quoted', 'changes_requested'].includes(item.status);
    const proofDecisionPending = item.paymentStatus === 'paid' && ['proof_review', 'changes_requested'].includes(item.status);
    const decisionStage = item.status === 'changes_requested' ? (proofDecisionPending ? 'proof_review' : quoteDecisionPending ? 'quoted' : item.status) : item.status;
    const active = Math.max(0, stages.indexOf(decisionStage));

    return (
        <>
            <Head title={`Production request - ${item.designName}`} />
            ...
            <div className="min-h-screen bg-[#f8f9fc] text-slate-950">
                <header className="border-b border-slate-200 bg-white">
                    <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6">
                        <a href={storefront.shopifyStoreUrl} target="_top" className="flex items-center gap-3">
                            ...
                            <span>
                                <strong className="block">My production request</strong>
                                <small className="text-slate-500">{item.id}</small>
                            </span>
                        </a>
                ...
                <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
                    ...
                    {!['rejected', 'cancelled'].includes(item.status) && (
                        <div className="mt-6 overflow-x-auto rounded-3xl border border-slate-200 bg-white p-6">
                            <div className="flex min-w-[720px] items-start">
                                {stages.map((stage, index) => (
                                    <div key={stage} className="relative flex-1 text-center">
                                        <div className={`relative z-10 mx-auto grid h-8 w-8 place-items-center rounded-full text-xs font-black ${index <= active ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{index < active ? '✓' : index + 1}</div>
                                        {index < stages.length - 1 && <span className={`absolute left-1/2 top-4 h-0.5 w-full ${index < active ? 'bg-indigo-600' : 'bg-slate-200'}`} />}
                                        <p className="relative z-10 mt-2 text-[10px] font-bold text-slate-500">{labels[stage]}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            {item.quote && (
                                <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <p className="text-xs font-bold uppercase tracking-wider text-violet-600">Quotation v{item.quote.version}</p>
                            ...
                            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                                ...
                                <div className="mt-4 space-y-3 text-sm text-slate-600">
                                    ...
                                    <p>
                                        <strong className="text-slate-900">Pattern:</strong> {item.summary?.pattern}
                                    </p>
                                    <p>
                                        <strong className="text-slate-900">Artwork:</strong> {item.summary?.artwork}
                                    </p>
                                </div>
                            </section>
                        </div>
