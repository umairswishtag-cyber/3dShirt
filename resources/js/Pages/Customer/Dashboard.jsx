...
export default function Dashboard({ storefront, catalog = [], productionRequests = [] }) {
    ...
    const openConfigurator = (designId = null) => {
        const baseUrl = usesShopifyIdentity
            ? storefront.shopifyConfiguratorUrl
            : storefront.configuratorUrl;
        const url = designId
            ? `${baseUrl}?design=${encodeURIComponent(designId)}`
            : baseUrl;
            ...
            <div className="min-h-screen bg-[#f8f9fc] text-slate-950">
                ...
                <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
                    <div className="relative flex min-h-[250px] flex-col justify-center overflow-hidden rounded-[28px] border border-indigo-100 bg-[linear-gradient(112deg,#fff_0%,#fbfbff_51%,#eef1ff_100%)] p-7 shadow-[0_14px_38px_rgba(49,46,129,0.06)] sm:p-10">
                        ...
                        <div className="relative z-10 max-w-xl"><span className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-violet-600"><UiIcon name="sparkles" className="h-4 w-4" />Customer workspace</span><h1 className="mt-3 text-4xl font-black tracking-[-0.035em] sm:text-[42px]">Hello, {customer?.name}</h1><p className="mt-3 text-sm leading-6 text-slate-500">Continue a draft or bring a new garment idea to life in your private design workspace.</p><button onClick={() => openConfigurator()} className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:shadow-xl"><UiIcon name="plus" className="h-4 w-4" />Create new design</button></div>
                    </div>

                    <section id="production-requests" className="mt-9">
                        <div className="flex items-end justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.2em] text-violet-600">Custom production</p><h2 className="mt-2 text-2xl font-black tracking-tight">Requests & quotations</h2><p className="mt-1 text-sm text-slate-500">Track review, approve quotations, clear payment, and follow printing.</p></div>{productionRequests.length > 0 && <span className="rounded-full bg-white px-3 py-1.5 text-xs font-bold text-slate-500 shadow-sm ring-1 ring-slate-200">{productionRequests.length}</span>}</div>
                        {productionRequests.length === 0 ? <div className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center text-sm text-slate-500">Published custom designs can be submitted to the team for technical review and quotation.</div> : <div className="mt-5 grid gap-4 md:grid-cols-2">{productionRequests.map((item) => <button key={item.id} onClick={() => router.visit(item.url)} className="rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start justify-between gap-4"><div><p className="font-black">{item.designName}</p><p className="mt-1 text-xs text-slate-500">{item.productName} · Qty {item.quantity}</p></div><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold capitalize text-indigo-700">{item.status.replaceAll('_', ' ')}</span></div>{item.quote && <p className="mt-4 text-sm font-black">{item.quote.currency} {item.quote.total}</p>}</button>)}</div>}
                    </section>
                            ...
                            {designs.map((design) => {
                                return (
                                    <SavedDesignCard
                                        ...
                                        design={design}
                                        catalog={catalog}
                                        previewActive={previewDesignId === design.id}
                                        onTogglePreview={() => setPreviewDesignId((id) => id === design.id ? null : design.id)}
                                        onOpen={() => openConfigurator(design.id)}
